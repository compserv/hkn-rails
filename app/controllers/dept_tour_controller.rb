class DeptTourController < ApplicationController
  def signup
    if request.post?
      ...
      @errors[:date]               = "Invalid date or time specified. Must be in the future, and between 11 (11am) and 17 (5pm)"   unless params[:date] && valid_date?(params[:date])
  ...
  private
    def valid_date?(date)
      ...
      date_valid = dt <= DateTime.new(2017, 5, 12) or dt >= DateTime.new(2017, 8, 23)
